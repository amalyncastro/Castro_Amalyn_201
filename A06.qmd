...
```{r}
bears_spread <- bears
opponent_spread <- opponent
result_sub <- bears_spread - opponent_spread

max_index <- which.max(result_sub)

# Get values
max_spread <- result_sub[max_index]
bears_score <- bears_spread[max_index]
opponent_score <- opponent_spread[max_index]

sprintf("The largest win spread is %d, when the Bears scored %d points and the Opponent scores %d points.", max_spread, bears_score, opponent_score)
```
...
```{r}
new_games <- matrix(c(37, 9, 17, 17), 2, 2)
...
for (row in 1:nrow(game_score)) {
  bears_score <- game_score[row, 1] 
  opponent_score <- game_score[row, 2] 
...
for (row in 1:nrow(game_score)) {
  ...
  win <- c(win, bears_score > opponent_score)
}
print(win)
```
...
```{r}
game_score <- matrix(c(game_score[, 1], game_score[, 2], win), nrow = nrow(game_score), ncol = 3)
colnames(game_score) <- c("bears", "opponent", "win")
print(game_score)
```
...
for (row in 1:nrow(game_score)) {
  if (game_score[row, 3] == 1) {
    # Stop counting once the first win is found
    break
  } else {
    losses <- losses + 1
  }
...
number_of_factors <- function(num) {
  ...
  return(length(factors)) # Number of factors
}

response <- 8
print(number_of_factors(response))
```
...
the_area <- calculate_area(l=102, w=60)

sprintf("The area in square inches is %.2f", the_area[1])
sprintf("The area in square feet is %.2f", the_area[2])
```
...
most_words <- function(vec=c()) {
  max_length <- max(nchar(vec)) 
  result <- vec[nchar(vec) == max_length]
  return(result)
}

output <- most_words(fruits)
print(output) 
```
...
is_vector <- function(val, vec) {
  ...
  for (i in seq_along(vec)) {
    if (val == vec[i]) {
      indicator <- TRUE
      return(sprintf("%s is in the vector.", val))
    }
  }
  if (!indicator) {
    return(sprintf("%s is not in the vector.", val))
  }
}

response <- is_vector(value, check)
print(response)
```
...
```{r}
convert_negatives <- function(mat) {
  mat[mat < 0] <- NA
  return(mat)
}

response <- convert_negatives(x)
print(response)
```
...
```{r}
convert_even_and_odd <- function(mat) {
  temp <- mat # Copy
...
response <- convert_even_and_odd(x)
print(response)
```
